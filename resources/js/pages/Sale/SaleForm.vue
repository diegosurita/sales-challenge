...
const addedProductIds = computed(
    () => new Set(form.products.map((product) => product.product_id)),
);
const addedServiceIds = computed(
    () => new Set(form.services.map((service) => service.service_id)),
);

const availableProducts = computed(() =>
    props.products.filter((product) => !addedProductIds.value.has(product.id)),
);
const availableServices = computed(() =>
    props.services.filter((service) => !addedServiceIds.value.has(service.id)),
);
...
const filteredClients = computed(() =>
    clientQuery.value === ''
        ...
        : props.clients.filter((c) =>
              c.name.toLowerCase().includes(clientQuery.value.toLowerCase()),
          ),
);
...
const filteredAvailableServices = computed(() =>
    serviceQuery.value === ''
        ...
        : availableServices.value.filter((service) =>
              service.name
                  .toLowerCase()
                  .includes(serviceQuery.value.toLowerCase()),
          ),
);
...
const addService = () => {
    ...
    if (
        ...
        !addedProductIds.value.has(requiredProduct.id)
    ) {
        const inStock = props.products.find(
        ...
        <form class="space-y-6" @submit.prevent="submit">
                ...
                <div class="relative mt-1">
                    <Combobox v-model="selectedClient" nullable by="id">
                        <div class="relative">
                            <ComboboxInput
                                id="client_combobox"
                                class="block w-full rounded-lg border border-slate-300 bg-white py-2 pr-10 pl-3 text-slate-900 transition outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
                                :display-value="
                                    (client) =>
                                        (client as Client | null)?.name ?? ''
                                "
            ...
            <div>
                ...
                <div class="flex gap-2">
                    <div class="relative flex-1">
                        <Combobox v-model="selectedProduct" nullable by="id">
                            <div class="relative">
                                ...
                                >
                                    <i
                                        class="pi pi-angle-down text-slate-400"
                                    />
                                </ComboboxButton>
                            </div>
                            <ComboboxOptions
                    ...
                    <button
                        ...
                        class="inline-flex items-center justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:cursor-pointer hover:bg-slate-50 focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        @click="addProduct"
                    >
                        Add
                    </button>
            ...
            >
                ...
                <div class="space-y-1 text-sm">
                    ...
                    <div
                        class="mt-2 flex justify-between border-t border-slate-300 pt-2 font-semibold text-slate-900"
                    >
                        <span>Total</span>
            ...
            >
                <ul class="space-y-1 text-sm text-red-600">
                    <li v-if="form.errors.client_id">
                        {{ form.errors.client_id }}
                    </li>
                    <li v-if="form.errors.products">
                        {{ form.errors.products }}
                    </li>
                    <li v-if="form.errors.services">
                        {{ form.errors.services }}
                    </li>
                    <li v-if="formErrors.items">{{ formErrors.items }}</li>
                </ul>
            </div>
